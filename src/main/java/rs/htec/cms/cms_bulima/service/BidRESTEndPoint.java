...
public class BidRESTEndPoint {
    ...
    public Response getBidsById(@HeaderParam("authorization") String token, @Context HttpServletRequest request, @PathParam("id") long id) {
        ...
        try {
            ...
            helper.setResponseToHistory(history, new DataNotFoundException("Bid at index " + id + " does not exist.."), em);
...
//    }
    ...
    @Path("/")
    @Produces(MediaType.APPLICATION_JSON)
    public Response getBids(@HeaderParam("authorization") String token, @Context HttpServletRequest request, @DefaultValue("1") @QueryParam("page") int page,
        ...
        if (bids.isEmpty()) {
            helper.setResponseToHistory(history, new DataNotFoundException("There is no Bids for this search!"), em);
